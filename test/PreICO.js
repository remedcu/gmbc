...
const PreICO = artifacts.require("./test_helpers/campaigns/PreICO.sol");
...
async function instantiate()
{
        ...
        const ico = await PreICO.new(token.address, role.beneficiary, role.team, role.game, role.advisors, role.bounty, role.founders, {from: role.owner2});
    ...
    it("Check Values", async function() {
        ...
        assert.equal(await ico.icoStartTime(), (new Date('December 25, 2017 8:00:00 GMT+3')).getTime() / 1000);
        assert.equal(await ico.icoEndTime(), (new Date('December 29, 2017 23:59:59 GMT+3')).getTime() / 1000);
